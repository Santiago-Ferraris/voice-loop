...
def describe_menu(payload: Mapping[str, Any]) -> str:
    ...
    if isinstance(plan, str) and plan.strip():
        ...
        return f"pide aprobar un plan: {heading}" if heading else "pide aprobar un plan"
    ...
    if labels:
        enumerated = ", ".join(
            f"{number_word(index)}: {label}" for index, label in enumerate(labels, start=1)
        )
        parts.append(f"Opciones: {enumerated}")
    if len(entries) > 1:
        extra = len(entries) - 1
